...
class decode:
    ...
    def codeblockextract(self,encodedseq):
        ...
        for idx,char in enumerate(encodedhash):
            seed += char
            confirmstr = account('0',0).bufferstr(seed)
    ...
    def decode(self,seed,codeblock):
        if type(seed) is str:
            ...
            matrix = self.matrix_reconstruct(seed)
        #for encoded,combi in matrix.items():
        #    print(encoded,'\t',combi)
        combi_list = []
...
class account:
    def __init__(self,account,state):
        self.accountdir = '/storage/account'
        self.encode = encode
    ...
    def passwd(self,extra):
        ...
        match self.state:
            case 0: #Sign up
                passwd = '123456'
                salt_gr = self.salt_generate()
                self.salt_str = salt_gr[1]
                self.salt_hash = salt_gr[0]
    ...
    def login(self,loginblock):
        import os
        #   loginblock = [<encodedaccname>,<encodedpwd>]
        #   Decode accname
        #   Check accountfile
        #   Yes: Extract salt and hash string on .account file
        #   Decode and hash pwd
        #   Compare pwd
        #   Return results
