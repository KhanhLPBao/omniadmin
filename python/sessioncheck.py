        ...
        for file in totalfiles:
            ...
            for jobs in totaljobs:
                with open(f'{signaldir}/{jobs}/{session}/out/{file}.status') as tmp_fs:
                    ...
                    tmp_fs.close()
                match filestatus:
                    case 0:     #waiting to be analyzed
                        job_status.append(f'{jobs} w')
                    case 1:     #file is processing
            ...
            for js in job_status:
                ...
                match stt:
                    case 'e':
                        job_status.append(f'{file} e')
                        break
                    case 'w':
                        job_status.append(f'{file} w')
                        waiting = True
                        break
                    case 'd':
                        if waiting:
...
def sessionsuccess():
    ...
    with open(f'{storagedir}/tempstorage/{session}/{filename}.json','w') as tmp_new:
        json.dump(fileprogress,tmp_new)
        tmp_new.close()
    p.run(f"mv -f {storagedir}/queqe/{filename}.fq.gz {storagedir}/{session}/tmpstorage",\
            shell=True,stdout=p.null, stderr=p.null)
